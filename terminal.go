...
func (t *Terminal) PrintBoards(myShips, opponentShots, myShots map[Coordinate]string, team string) {
	...
	// Print column headers for both boards (adjusted for 2-digit row numbers)
	...
	// Print top borders (adjusted for 2-digit row numbers)
	fmt.Fprintf(t.output, "   %s%s   %s\n", strings.Repeat("-", 20), spaceWidth, strings.Repeat("-", 20))

	// Print rows for both boards
	for row := 0; row < 10; row++ {
		// Print row number and first board (1-10 instead of 0-9, right-aligned for consistent spacing)
		...
		// Print row number and second board (1-10 instead of 0-9, right-aligned for consistent spacing)
		fmt.Fprintf(t.output, "%2d|", row+1)
		for col := 0; col < 10; col++ {
			coord := Coordinate{X: col, Y: row}
		...
		// Print bottom borders (adjusted for 2-digit row numbers)
		fmt.Fprintf(t.output, "   %s   %s%s\n", strings.Repeat("-", 20), spaceWidth, strings.Repeat("-", 20))
	}
}
